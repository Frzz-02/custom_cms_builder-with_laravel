    ...
    <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6 mb-6">
        <div class="flex items-center gap-4">
            ...
            <div>
                ...
                <p class="text-slate-600 mt-1">Update testimonial information</p>
            </div>
        </div>
    </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6 space-y-6">
                    ...
                    <div>
                        <label for="image" class="block text-sm font-semibold text-slate-700 mb-2">
                            {{ $testimonial->image ? 'Change Photo' : 'Customer Photo' }}
                        </label>
                        <input type="file" 
                               id="image" 
                               name="image" 
                    ...
                    <div>
                        ...
                        <textarea id="content" 
                                  ...
                                  placeholder="Customer's feedback or testimonial...">{{ old('content', $testimonial->content) }}</textarea>
                        @error('content')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
                    </div>
                ...
                <div class="bg-white rounded-lg shadow-sm border border-slate-200 p-6 space-y-6 sticky top-24">
                    ...
                    <div>
                        ...
                        <select id="star" 
                            ...
                            <option value="5" {{ old('star', $testimonial->star) == 5 ? 'selected' : '' }}>⭐⭐⭐⭐⭐ (5 Stars)</option>
                            <option value="4" {{ old('star', $testimonial->star) == 4 ? 'selected' : '' }}>⭐⭐⭐⭐ (4 Stars)</option>
                            <option value="3" {{ old('star', $testimonial->star) == 3 ? 'selected' : '' }}>⭐⭐⭐ (3 Stars)</option>
                            <option value="2" {{ old('star', $testimonial->star) == 2 ? 'selected' : '' }}>⭐⭐ (2 Stars)</option>
                            <option value="1" {{ old('star', $testimonial->star) == 1 ? 'selected' : '' }}>⭐ (1 Star)</option>
                        </select>
                        @error('star')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
