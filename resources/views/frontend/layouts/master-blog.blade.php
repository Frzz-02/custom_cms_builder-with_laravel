{{--
    master-blog.blade.php
    Layout for individual blog post detail pages.
    Features: reading progress bar, estimated read time slot,
    scroll-reveal, share buttons area, related posts, flash toasts.

    Child view usage:
        @extends('frontend.layouts.master-blog')
        @section('title', $blog->meta_title ?? $blog->title)
        @section('description', $blog->meta_description ?? $blog->excerpt)
        @section('og_image', asset('storage/' . $blog->thumbnail))
...
<head>
    ...
    <style>
        ...
        .blog-content h2 {
            ...
            color: #111827; margin: 2.5rem 0 1rem;
            padding-bottom: .5rem; border-bottom: 2px solid #f3f4f6;
        }
        .blog-content h3 {
            font-size: 1.25rem; font-weight: 600;
            color: #111827; margin: 2rem 0 .75rem;
        }
        .blog-content p    { margin-bottom: 1.5rem; }
        .blog-content ul,
        .blog-content ol   { padding-left: 1.75rem; margin-bottom: 1.5rem; }
        ...
        .blog-content img  {
            ...
            margin: 2rem 0; box-shadow: 0 4px 20px rgba(0,0,0,0.08);
        }
        ...
        .blog-content pre {
            ...
            overflow-x: auto; margin: 2rem 0; font-size: .9rem;
        }
        ...
        .blog-content th,
        .blog-content td   { border:1px solid #e5e7eb; padding:.6rem 1rem; font-size:.95rem; }
...
<body class="bg-white text-gray-900 antialiased">
    ...
    <!-- Pure JS: page reveal + reading bar + scroll-reveal + share + scroll-to-top -->
    <script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        // ── Estimated read time ──────────────────────────────────────────
        var contentEl = document.querySelector('.blog-content');
        var readTimeEl = document.getElementById('read-time');
        if (contentEl && readTimeEl) {
            var words = contentEl.innerText.trim().split(/\s+/).length;
